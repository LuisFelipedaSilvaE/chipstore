...
$dal = new ProdutoDal();
$id = $_GET['id'];
if (isset($_SESSION['conteudo-editando-produto-erro'])) {
  $produtoParaEditar = $_SESSION['conteudo-editando-produto-erro'];
} else {
  $produtoParaEditar = $dal->findById($id);
}
?>
<!DOCTYPE html>
<html lang="pt-br">
  ...
  <main class="flex-1 overflow-hidden p-6">
    <header class="flex gap-4 items-center">
      ...
      <div>
        ...
        <h2 class="text-gray-400">Edite um produto do catálogo</h2>
      </div>
    </header>
    <div class="flex flex-col gap-4 mt-4 bg-(--secondary-bg-color) rounded-2xl p-6 border border-gray-800">
      <?php if (isset($_SESSION['msg-erro-editando-produto'])): ?>
      ...
      <?php
        unset($_SESSION['msg-erro-editando-produto']);
      endif;
      ?>
      <?php if (isset($_SESSION['msg-erro-editando-produto-sku-invalido'])): ?>
        <div class="error-message-container flex gap-2 items-center justify-between bg-red-600/10 border border-red-600/50 rounded text-red-600 px-2 py-1">
          SKU já registrado. Informe outro valor.
          <i class="error-message fa fa-times cursor-pointer p-1"></i>
        </div>
      <?php
        unset($_SESSION['msg-erro-editando-produto-sku-invalido']);
      endif;
      ?>
              ...
              <div class="flex flex-col justify-center gap-1">
                ...
                <select id="categoria" name="categoria" class="min-w-60 w-full px-2 py-1 rounded-lg border border-gray-800 bg-(--input-bg-color) text-white focus:ring-(--main-color) focus:border-(--main-color) outline-none transition-colors" required>
                  ...
                  <option value="Processadores" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Processadores' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Processadores</option>
                  <option value="Placas-mãe" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Placas-mãe' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Placas-mãe</option>
                  <option value="Placas de Vídeo" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Placas de Vídeo' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Placas de Vídeo</option>
                  <option value="Memórias RAM" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Memórias RAM' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Memórias RAM</option>
                  <option value="Armazenamento" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Armazenamento' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Armazenamento (SSD/HDD)</option>
                  <option value="Fontes de Alimentação" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Fontes de Alimentação' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Fontes de Alimentação</option>
                  <option value="Refrigeração" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Refrigeração' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Refrigeração (Coolers/Water Coolers)</option>
                  <option value="Gabinetes" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Gabinetes' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Gabinetes</option>
                  <option value="Monitores" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Monitores' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Monitores</option>
                  <option value="Periféricos" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Periféricos' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
                    ?>>Periféricos (Mouse, Teclado, Headset)</option>
                  <option value="Acessórios" class="bg-(--input-bg-color)"
                    <?php echo isset($_GET['id']) && 'Acessórios' == $produtoParaEditar->getCategoria()
                      ? 'selected'
                      : ''
      ...
      <?php
      unset($_SESSION['conteudo-editando-produto-erro']);
      ?>
    </div>
  </main>
